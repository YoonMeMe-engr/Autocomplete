...
autoCompleteInputTag.addEventListener("keyup" , (event) => {
  if (
    event.key === "ArrowDown" || //||or operator က ၃ ခုထဲက ၁ခုခုမှန်ရင် အထဲက code ကို run မယ်
    event.key === "ArrowUp" ||
    event.key === "Enter"
  ) {
    nevigateAndSelectProduct(event.key);
    return;
  };
  ...
  const hasToShowProduct = filterProduct.length > 0;
  if (hasToShowProduct) {
...
const nevigateAndSelectProduct = (key) => {
  if (key === "ArrowDown") {
    ...
    const productItemContainerToSelect = selectProduct(indexToSelect);
    productItemContainerToSelect.classList.add("selected");
  } else {       //(key === "Enter") က ထည့်မရေးလဲရ
    const enterProduct = selectProduct(indexToSelect);
  }
};

const selectProduct = (index) => {
  const productIdToSelect = filterProduct[indexToSelect].id.toString();
  ...
  return productItemContainerToSelect; // return ပြန်ပေးရတယ်
};
